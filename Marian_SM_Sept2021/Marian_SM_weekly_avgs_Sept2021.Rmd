---
title: "Marian_SM_weekly_avgs_Sept2021"
author: "Joe Murphy"
date: "11/29/2021"
...
## Latest Marian Compilation Data (Sept 2021)

```{r}

# Marian

Marian_SM_Sept2021 <- readr::read_csv("compiled_Marian_Sept2021.csv", col_types = cols(Date = col_character(), .default = col_double())) %>% 
...
Marian_SM_good_data_Sept2021 <- Marian_SM_Sept2021 %>%
...
* To align weekly start dates with Rock Creek, for JTM thesis, set variable 'start_day_of_week = 7'
...
weekly_Marian_SM_toSept2021 <- temp_agg_meadow_dat(Marian_SM_good_data_Sept2021, interval = "30 min", start_day_of_week = 7, data.thresh = 25) %>% 
  dplyr::mutate(across(starts_with("VWC"), ~round(.x, digits = 3)))

# write out to CSV

weekly_Marian_SM_toSept2021 %>% 
    mutate(date = as.character(date)) %>%
    rename(Date = date) %>%
    readr::write_csv(path = "weekly_SM_Marian_Sept2021.csv")
