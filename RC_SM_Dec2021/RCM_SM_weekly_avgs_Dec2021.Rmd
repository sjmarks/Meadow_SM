...
## Latest RCM Compilation Data (Dec 2021)

```{r}

# Rock Creek, for function testing purposes

RC_SM_Dec2021 <- readr::read_csv("compiled_ver2_RC_Dec2021.csv", col_types = cols(Date = col_character(), .default = col_double())) %>% 
...
RC_SM_good_data_Dec2021 <- RC_SM_Dec2021 %>%
...
weekly_RC_SM_toDec2021<- temp_agg_meadow_dat(RC_SM_good_data_Dec2021, interval = "30 min", start_day_of_week = 7)

# write out to CSV

weekly_RC_SM_toDec2021 %>% 
    mutate(date = as.character(date)) %>%
    rename(Date = date) %>%
    readr::write_csv(path = "weekly_SM_RC_Dec2021.csv")
