---
title: "RCM_SM_weekly_avgs_Jun2022"
author: "Joe Murphy"
date: "09/30/2022"
...
## Latest RCM Compilation Data (Jun 2022)

```{r}

# Rock Creek, for function testing purposes

RC_SM_Jun2022 <- readr::read_csv("compiled_ver2_RC_Jun2022.csv", col_types = cols(Date = col_character(), .default = col_double())) %>% 
...
RC_SM_good_data_Jun2022 <- RC_SM_Jun2022 %>%
...
weekly_RC_SM_toJun2022<- temp_agg_meadow_dat(RC_SM_good_data_Jun2022, interval = "30 min", start_day_of_week = 7)
```

```{r}
# write out to CSV

weekly_RC_SM_toJun2022 %>% 
    mutate(date = as.character(date)) %>%
    #rename(Date = date) %>%
    readr::write_csv(path = "weekly_SM_RC_Jun2022.csv")
