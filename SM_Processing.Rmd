...
This demo is intended to accompany the `.R` script as a README for compiling soil moisture raw data files collected at the Chester, CA meadows- Marian, Control, Rock Creek, and Childs using HOBO Onset and Campbell Sci. data loggers. The purpose of this document is to explain the functions and demonstrate how to use them. This will ease potential troubleshooting and open the door for code to be adapted and improved in the future if desired.  
...
`read_soilsapflow` reads in the .dat files produced by the Campbell Scientific CR1000 and CR300 logger programs currently collecting SoilVUE10 soil moisture data in Rock Creek Meadow.

**Arguments:**

* `path` path to the .dat soil and sapflow file produced by the CR1000. Must be quoted and end with .dat
...
`readSoilvue_CR1000` manipulates the data frame `tbl_df` produced by `read_soilsapflow` to include only timestamp and VWC variables, navigating electrical conductivity, permitivity, temperature, and sap flow data currently produced by the CR1000 soil and sap flow program

**Arguments:**

* `data` A data frame `tbl_df` produced by `read_soilsapflow`
