...
#### Prev. Rock Creek Compile (December 2021)

```{r}

RC_compile_Dec2021 <- readr::read_csv(file = "RC_SM_Dec2021/compiled_ver2_RC_Dec2021.csv", 
                                       col_types = cols(Date = col_character(), .default = col_double())) %>% 
  dplyr::mutate(Date = lubridate::ymd_hms(Date))

```

#### New Compile for RC (April 2022)

```{r}

compiled_RC_Apr2022 <- process_SM_data(path = "RC_SM_Apr2022", 
                                        site = "RC", prev_compile = RC_compile_Dec2021, written_file_name = "compiled_RC_Apr2022.csv")
```
#### Data Cleaning Extra Step (April 2022)

```{r}

RC_compile_ver2_Apr2021 <- readr::read_csv(file = "RC_SM_Apr2022/compiled_RC_Apr2022.csv", 
                                       col_types = cols(Date = col_character(), .default = col_double()))%>%
      dplyr::distinct()%>%

readr::write_csv(., path = paste0("RC_SM_Apr2022", "/", written_file_name = "compiled_ver2_RC_Apr2022.csv"))
```
