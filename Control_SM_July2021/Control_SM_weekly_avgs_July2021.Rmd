---
title: "Control_SM_weekly_avgs_July2021"
author: "Simon Marks"
date: "7/21/2021"
...
Control_SM_July2021 <- readr::read_csv("Control_SM_July2021/compiled_Control_July2021.csv", col_types = cols(Date = col_character(), .default = col_double())) %>% 
...
* VWC values less than or equal to 0 or greater than 0.7

```{r}

Control_SM_good_data_July2021 <- Control_SM_July2021 %>%
...
**First week is week of 2019-07-12, last week is week of 2021-6-25*

```{r}

weekly_Control_SM_toJuly2021 <- temp_agg_meadow_dat(Control_SM_good_data_July2021, interval = "30 min", start_day_of_week = 6, data.thresh = 25) %>% 
  dplyr::mutate(across(starts_with("VWC"), ~round(.x, digits = 3)))

# write out to CSV

weekly_Control_SM_toJuly2021 %>% 
    mutate(date = as.character(date)) %>%
    rename(Date = date) %>%
    readr::write_csv(path = "Control_SM_July2021/weekly_SM_Control_July2021.csv")
