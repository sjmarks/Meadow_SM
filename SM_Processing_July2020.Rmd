...
#### Helper- `read_soilsapflow`

```{r}

read_soilsapflow <- function(path) {
...
compiled_RC_July2020 <- process_SM_data(path = "C:/Users/Simon/Desktop/Meadow_SM/RC_SM_July2020", 
                                        site = "RC", written_file_name = "compiled_RC_July2020.csv")


```

John's data- added to RC compilation on 8/11/2020

```{r}
Jasbinsek_data1 <- read_soilsapflow("RC_SM_July2020/CR800Series_Table2_JasbinsekData.dat") %>% 
  select(Date = TIMESTAMP, VWC_18in_RCSM2_JJ = VWC)

Jasbinsek_data2 <- read_soilsapflow("RC_SM_July2020/CR800Series_Table3_JasbinsekData.dat") %>% 
  select(Date = TIMESTAMP, VWC_18in_2_RCSM2_JJ = VWC_2)

John_RCSM2_data <- dplyr::left_join(Jasbinsek_data1, Jasbinsek_data2, by = "Date") %>% 
  mutate(Date = as.POSIXct(Date, tz = "UTC"))


existing_RC_compilation <- readr::read_csv("RC_SM_July2020/compiled_RC_July2020.csv", col_types = cols(Date = col_character(), .default = col_double())) %>% 
  mutate(Date = as.POSIXct(Date, tz = "UTC"))

updated_RC_compilation <- dplyr::left_join(existing_RC_compilation, John_RCSM2_data, by = "Date") %>% 
  mutate(Date = as.character(Date)) %>% 
  readr::write_csv("RC_SM_July2020/compiled_RC_July2020_plusJJdata.csv")
...
compiled_Marian_July2020 <- process_SM_data(path = "C:/Users/Simon/Desktop/Meadow_SM/Marian_SM_July2020", site = "Marian", 
...
#### Control Soil Moisture Data

Note that CMSM2 weirdly recorded data past the collection date. Need to replace this logger I think. Deleted all records that extended past 07/07/2020 when the data was collected. 

```{r}

compiled_Control_July2020 <- process_SM_data(path = "C:/Users/Simon/Desktop/Meadow_SM/Control_SM_July2020", 
                                        site = "control", written_file_name = "compiled_Control_July2020.csv")
...
```{r}
compiled_childs_July2020 <- process_SM_data(path = "C:/Users/Simon/Desktop/Meadow_SM/Childs_SM_July2020", 
                                        site = "childs", written_file_name = "compiled_Childs_July2020.csv")
