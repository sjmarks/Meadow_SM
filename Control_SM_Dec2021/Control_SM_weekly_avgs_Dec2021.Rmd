---
title: "Control_SM_weekly_avgs_Dec2021"
author: "Joe Murphy"
date: "02/04/2022"
...
## Latest Control Compilation Data (Dec 2022)

```{r}

# Control

Control_SM_Dec2021 <- readr::read_csv("compiled_Control_Dec2021.csv", col_types = cols(Date = col_character(), .default = col_double())) %>% 
...
Control_SM_good_data_Dec2021 <- Control_SM_Dec2021 %>%
...
weekly_Control_SM_toDec2021 <- temp_agg_meadow_dat(Control_SM_good_data_Dec2021, interval = "30 min", start_day_of_week = 7, data.thresh = 25) %>% 
  dplyr::mutate(across(starts_with("VWC"), ~round(.x, digits = 3)))

# write out to CSV

weekly_Control_SM_toDec2021 %>% 
    mutate(date = as.character(date)) %>%
    rename(Date = date) %>%
    readr::write_csv(path = "weekly_SM_Control_Dec2021.csv")
